...
def show():
    ...
    if 'backtest_result' in st.session_state:
        show_backtest_results(st.session_state['backtest_result'])


def run_backtest(strategy, start_date, end_date, symbols, initial_capital, 
                bar_interval, slippage, commission):
    """运行回测"""
...
def show_backtest_results(result):
    ...
    # 资金曲线图
    st.markdown("### 📈 资金曲线")
    show_equity_curve(result)


def show_equity_curve(result):
    """显示资金曲线"""
    ...
    try:
        ...
        # 从数据库获取资金曲线数据
        equity_data = analyzer.db.execute_query(
            """
            SELECT timestamp, total_assets, total_pnl_pct, max_drawdown
            FROM equity_curve
            WHERE account_id = ?
            ORDER BY timestamp
            """,
        ...
        if equity_data:
            df = pd.DataFrame(equity_data, columns=['timestamp', 'total_assets', 'pnl_pct', 'drawdown'])
            ...
            st.warning("⚠️ 暂无资金曲线数据")
    
    except Exception as e:
        st.error(f"❌ 绘制资金曲线失败: {str(e)}")
