...
def show_realtime_quotes():
    ...
    def color_pct(val):
        if isinstance(val, str) and '%' in val:
            if '+' in val:
                return 'background-color: #ffe6e6; color: #ff0000'
            elif '-' in val:
                return 'background-color: #e6ffe6; color: #00ff00'
        return ''
    
    styled_df = quotes_df.style.applymap(color_pct, subset=['涨跌幅'])
